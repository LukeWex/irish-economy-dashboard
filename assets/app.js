...
(async () => {
  try {
    ...
    if (un) setKPI('kpi-unemp', (un.latest * 1).toFixed(1) + '%', un.latest_date);
    ...
    if (gni && gni.latest_value != null) setKPI('kpi-gnistar', fmtEURm(gni.latest_value), gni.latest_year);

    const wages = snap.series.wage_growth;
    if (wages && wages.latest_yoy != null) setKPI('kpi-wages', (wages.latest_yoy * 100).toFixed(1) + '%', wages.latest_period);

    // ---- Charts ----
    if (snap.series.mdd)            plotLine('chart-mdd',       snap.series.mdd.x,              snap.series.mdd.y,              'MDD (2018=100)', '');
    if (snap.series.current_account) plotBars('chart-ca',       snap.series.current_account.x,  snap.series.current_account.y,  'Current account (€bn)', '€bn');
    if (snap.series.employment_rate) plotLine('chart-emp-rate', snap.series.employment_rate.x,  snap.series.employment_rate.y,  'Employment rate (15–64)', '%');
    if (snap.series.live_register)   plotLine('chart-lr',       snap.series.live_register.x,    snap.series.live_register.y,    "Live Register (SA, '000)", '000s');

    const h = snap.series.housing || {};
    if (h.completions && h.permissions) {
